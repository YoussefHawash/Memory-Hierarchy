...
let m_z = 0x05010902; // must not be zero, nor 0x9068ffff
let cycles = 0;
let l1wb = 0;
let l2wb = 0;
...
export function AccessCacheL2(
  ...
  if (Cache2[Index][randInd].dirty) {
    // Write back to DRAM if dirty
    cycles += 50; // Write back cost
    l2wb++;
  }

  Cache2[Index][randInd].tag = Tag;
  Cache2[Index][randInd].valid = true;
  Cache2[Index][randInd].dirty = isWrite;
...
export function Simulator(lineSize: number, generator: number) {
  ...
  for (let i = 0; i < NUM_CYCLES; i++) {
    ...
    if (p <= 0.35) {
      ...
      MemoryAccess(addr, lineSize, memAccess === memAccessType.Write);
    } else cycles++;
  }
...
export function reset() {
  ...
  l2_hit = 0;
  l2_miss = 0;
  m_w = 0xababab55; // must not be zero, nor 0x464fffff
  m_z = 0x05080902; // must not be zero, nor
  l1wb = 0;
  l2wb = 0;
}
